...
function AccountRequests() {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const res = await axios.patch(`${url}/requests/status/${id}`, { status }, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data.success) {
  ...
  const deleteRequest = async (id: string) => {
    ...
    try {
      const res = await axios.delete(`${url}/requests/remove/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data.success) {
        toast.success(res.data.msg);
    ...
    <div className="w-full min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
          ...
          {["all", "pending", "approved", "rejected"].map((f) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-semibold capitalize transition-all ${
                filter === f ? "bg-orange-500 text-white shadow-sm" : "bg-white text-gray-600 border border-gray-200 hover:border-orange-300"
              }`}
            >
              {f}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="bg-gray-100 p-5 rounded-full mb-4"><FaInbox className="w-8 h-8 text-gray-400" /></div>
            ...
            <div key={req._id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {req.status === "pending" && (
                    <>
                      <button onClick={() => updateStatus(req._id, "approved")} className="flex items-center gap-1.5 px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg transition-all">
                        <FaCheck size={12} /> Approve
                      </button>
                      <button onClick={() => updateStatus(req._id, "rejected")} className="flex items-center gap-1.5 px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-lg transition-all">
                        <FaTimes size={12} /> Reject
                      </button>
                    </>
                  )}
                  {req.status !== "pending" && (
                    <button onClick={() => updateStatus(req._id, "pending")} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-semibold rounded-lg transition-all">
                      Reset
                    </button>
                  )}
                  <button onClick={() => deleteRequest(req._id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Delete">
                    <FaTrash size={14} />
                  </button>
                </div>
              </div>
