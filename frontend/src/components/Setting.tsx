...
import { FaUserCircle, FaShieldAlt, FaChevronRight, FaBell, FaPalette, FaSignOutAlt } from "react-icons/fa";
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center justify-between p-5 bg-white rounded-xl border border-gray-200 shadow-sm transition-all duration-200 
               ${isActive ? "border-orange-500 ring-1 ring-orange-500" : "hover:border-orange-300 hover:shadow-md"}`
            }
          >
            <div className="flex items-center gap-5">
              <div className={`p-3 rounded-full ${option.color} flex items-center justify-center shrink-0`}>
                {option.icon}
              </div>
              <div>
                <h2 className="text-lg font-bold text-gray-800 group-hover:text-orange-600 transition-colors">{option.title}</h2>
