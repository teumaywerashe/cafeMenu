    ...
    <div className="w-full min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage your Platform Users and their Profiles</p>
        </div>
      ...
      <div className="hidden md:grid grid-cols-[3fr_3fr_3fr_3fr] gap-4 px-6 py-3 bg-gray-100 rounded-t-xl text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">
        <p>profileImage</p><p>Name</p><p>Email</p><p>Role</p>
      </div>

      <div className="space-y-4 md:space-y-0 bg-white rounded-b-xl shadow-sm border border-gray-200 md:border-t-0">
        {filteredUsers.length > 0 ? (
          filteredUsers.map((user: any, i: number) => (
            <div key={i} className="group gap-10 flex flex-col md:grid md:grid-cols-[3fr_3fr_3fr_3fr] p-4 border-b border-gray-500 last:border-b-0 hover:bg-orange-50/30 transition-colors capitalize duration-200 items-center">
              ...
              <div className="w-full">
                <p className="font-bold text-gray-800 text-lg md:text-base">{user.name}</p>
                <span className="md:hidden inline-block px-2 py-0.5 mt-1 rounded text-xs font-medium bg-gray-100 text-gray-600">{user.email}</span>
              </div>
              <div className="w-full"><p className="text-gray-500 text-sm line-clamp-2">{user.email}</p></div>
              <div className="w-full"><p className="text-gray-500 text-sm line-clamp-2">{user.role}</p></div>
            </div>
          ))
        ) : (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-400 text-sm">Try adjusting your search or check your internet connection</p>
          </div>
        )}
      </div>
    </div>
