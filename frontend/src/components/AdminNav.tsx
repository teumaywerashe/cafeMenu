...
import {
  ...
  FaUserCog, FaCog, FaBell, FaUserCircle,
} from "react-icons/fa";
...
function AdminNav() {
  ...
  const navLinks = [
    { name: "Dashboard", path: role === "admin" ? "/admin/dashboard" : "/superadmin/dashboard", icon: <FaTachometerAlt /> },
    { name: "Management", path: role === "admin" ? "/admin/management" : "/superadmin/management", icon: <FaUserCog /> },
    { name: "Settings", path: role === "admin" ? "/admin/setting" : "/superadmin/setting", icon: <FaCog /> },
  ];
      ...
      <div className="px-4 md:px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="flex items-center gap-2" onClick={() => navigate("/admin/")}>
            <img src={logo} alt="Logo" className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover border border-gray-100 hidden sm:block" />
            <span className="text-lg md:text-xl font-bold text-gray-800 tracking-tight">
              The Daily Feast{" "}
              <span className="text-orange-500 text-xs uppercase tracking-wider hidden lg:inline-block ml-1">{role}</span>
            </span>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden md:block text-left">
                <p className="text-xs font-bold capitalize text-gray-700 group-hover:text-orange-600">{user.name}</p>
            ...
            {isProfileOpen && (
              ...
              >
                ...
                <button
                  onClick={() => { navigate("/admin/profileSetting"); setIsProfileOpen(false); }}
                ...
                <button
                  onClick={() => { navigate("/admin/setting"); setIsProfileOpen(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-600 hover:bg-orange-50 hover:text-orange-600 flex items-center gap-2 transition-colors"
                >
                  <FaCog /> Settings
                </button>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                  isActive ? "bg-orange-500 text-white shadow-md shadow-orange-200" : "bg-white text-gray-600 hover:bg-gray-200"
                }`
              }
            >
