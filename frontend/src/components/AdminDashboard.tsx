...
function AdminDashboard() {
  ...
  const filteredList = userItems.filter((item: any) => {
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 tracking-tight">Menu Dashboard</h1>
          <p className="text-gray-500 text-sm mt-1">Manage your cafe's dishes and prices</p>
        </div>
          ...
          filteredList.map((item: any, i: number) => (
            ...
            >
              ...
              <div className="w-full">
                <p className="font-bold text-gray-800 text-lg md:text-base">{item.name}</p>
                <span className="md:hidden inline-block px-2 py-0.5 mt-1 rounded text-xs font-medium bg-gray-100 text-gray-600">{item.category}</span>
              </div>
              <div className="w-full">
              ...
              <div className="hidden md:block text-right">
                <p className="font-bold text-gray-900">{item.price * 10} Birr</p>
              </div>
            </div>
          ))
        ) : (
