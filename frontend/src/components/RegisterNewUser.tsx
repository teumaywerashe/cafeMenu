...
import { FaArrowLeft, FaEnvelope, FaEye, FaEyeSlash, FaLock, FaUserAlt } from "react-icons/fa";
...
function RegisterNewUser() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${url}/user/register`, { name, email, password });
    ...
    <div className="min-h-screen flex w-full bg-white font-sans">
      <div className="w-full flex items-center justify-center p-6 relative">
        <button onClick={() => navigate(-1)}
          className="absolute top-6 right-6 p-2 text-gray-400 hover:text-gray-800 hover:bg-gray-100 rounded-full transition-all" title="Go Back">
          <FaArrowLeft size={20} />
        </button>
        ...
        <div className="w-full max-w-md">
          <div className="mb-10 text-center md:text-left">
            ...
            <p className="text-gray-500">Please enter the detailed information of the user to access the admin panel.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 ml-1">User Name</label>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type="email" placeholder="admin@cafe.com"
                  className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
